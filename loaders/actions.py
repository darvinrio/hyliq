...
def get_actions(address: str, use_cache: bool = True) -> pl.DataFrame:
    ...
    df_actions = pl.read_csv(
        ...
        schema_overrides={
            **actions_schema,
            "time": pl.Int64  # Read as integer first
        },
        null_values=["-", "", "null", "NULL"],
        infer_schema_length=10000
    ).with_columns([
        pl.col("time").cast(pl.Datetime("ms"))  # Convert from milliseconds timestamp to datetime
    ])
    
    return df_actions

def generate_positions(address: str, use_cache: bool = True) -> tuple[pl.DataFrame, pl.DataFrame]:
