...
class AccountClassTransferTxModel(BaseModel):
    ...
    usdc: float = Field(..., description="USDC amount transferred")
    toPerp: bool = Field(..., description="True if transferring to Perp account, False if to Spot account")
...
class CStakingTransferTxModel(BaseModel):
    ...
    isDeposit: bool = Field(..., description="True if deposit (stake), False if withdrawal (unstake)")
